...
public:
    ...
    // 初始化函数，确保在使用前正确初始化websocket Initialize the function to ensure that the websocket is properly initialized before use
    void initialize(tcp::socket&& socket);

    static MessageSender& getInstance();

private:
    //  Modify the type of a member variable
